...
function extractValue(arr, key) {
    return arr.reduce(function(accum,nextAccum){
        accum.push(nextAccum[key]);
        return accum;
    },[]);
}
...
function vowelCount(str) {
    const vowels = 'aeiou';
    return str.toLowerCase().split('').reduce(function(accum,nextAccum){
        if(vowels.indexOf(nextAccum) !== -1){
            if(accum[nextAccum]){
                accum[nextAccum] ++;
            }else{
                accum[nextAccum] = 1;
...
function addKeyAndValue(arr, key, value) {
    return arr.reduce(function(accum,nextAccum,index){
...
function partition(arr, callback) {
    return arr.reduce(function(accum,nextAccum){
        if(callback(nextAccum)){
            accum[0].push(nextAccum);
        } else{
            accum[1].push(nextAccum);
        }
        return accum;
    },[[],[]]);
}
